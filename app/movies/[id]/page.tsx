 ...
 */
const SimilarMovieCard: React.FC<{ movie: any; index: number }> = ({ movie, index }) => {
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-xl bg-bg-card border border-glass-border hover:border-netflix-red/30 transition-all duration-500 hover:shadow-netflix/30">
        ...
        <div className="absolute inset-0 bg-gradient-backdrop opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
        
        <div className="absolute bottom-0 left-0 right-0 p-4 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 bg-gradient-to-t from-bg-primary/90 to-transparent">
          <h4 className="font-semibold text-text-primary mb-1 line-clamp-2">{title}</h4>
...
const DetailsContent: React.FC<{ details: DetailsResponse; type: MediaType }> = ({ details, type }) => {
  const detailItems = [
    ...
    ...(type === "movie" && (details as any).runtime ? [
      { label: "Runtime", value: formatRuntime((details as any).runtime) }
    ] : []),
    ...(type === "movie" && (details as any).budget ? [
      { label: "Budget", value: `$${(details as any).budget.toLocaleString()}` }
    ] : []),
    ...(type === "movie" && (details as any).revenue ? [
      { label: "Revenue", value: `$${(details as any).revenue.toLocaleString()}` }
    ] : []),
    { label: "Status", value: (details as any).status || "Released" },
              ...
              <div className="flex flex-wrap justify-center lg:justify-start items-center gap-3 sm:gap-4 md:gap-6 text-sm sm:text-base md:text-lg">
                ...
                {type === "movie" && (details as any).runtime && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white">{formatRuntime((details as any).runtime)}</span>
                  </div>
                )}
                  ...
                  >
                    ...
                    {similar && similar.length > 0 ? (
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4 lg:gap-6">
                        {similar.slice(0, 12).map((movie: any, index: number) => (
                          <motion.div
                            key={movie.id}
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ delay: index * 0.05, duration: 0.3 }}
                          >
                            <SimilarMovieCard movie={movie} index={index} />
                          </motion.div>
                        ))}
                      </div>
                    ) : (
