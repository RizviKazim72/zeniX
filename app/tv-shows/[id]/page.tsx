 ...
 */
const SimilarTVCard: React.FC<{ show: any; index: number }> = ({ show, index }) => {
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-slate-800/30 rounded-lg p-4 border border-slate-700/30">
            ...
            <dd className="text-white text-lg font-semibold mt-1">{(details as any).number_of_seasons}</dd>
          </div>
          <div className="bg-slate-800/30 rounded-lg p-4 border border-slate-700/30">
            <dt className="text-gray-400 text-sm font-medium">Episodes</dt>
            <dd className="text-white text-lg font-semibold mt-1">{(details as any).number_of_episodes}</dd>
          </div>
          <div className="bg-slate-800/30 rounded-lg p-4 border border-slate-700/30">
            <dt className="text-gray-400 text-sm font-medium">Status</dt>
            <dd className="text-white text-lg font-semibold mt-1">{(details as any).status}</dd>
          </div>
        </div>
      </div>
    )}
...
const DetailsContent: React.FC<{ details: DetailsResponse; type: MediaType }> = ({ details, type }) => {
  const detailItems = [
    ...
    ...(type === "tv" && (details as any).number_of_seasons ? [
      { label: "Seasons", value: (details as any).number_of_seasons },
      { label: "Episodes", value: (details as any).number_of_episodes },
      { label: "Episode Runtime", value: (details as any).episode_run_time?.length > 0 ? `${(details as any).episode_run_time[0]} min` : null },
    ] : []),
    { label: "Status", value: (details as any).status || "Ended" },
    { label: "Original Language", value: (details as any).original_language?.toUpperCase() },
    { label: "First Air Date", value: (details as any).first_air_date ? formatDate((details as any).first_air_date) : null },
    { label: "Last Air Date", value: (details as any).last_air_date ? formatDate((details as any).last_air_date) : null },
    ...(type === "tv" && (details as any).networks?.length > 0 ? [
      { label: "Networks", value: (details as any).networks.map((network: any) => network.name).join(", ") }
    ] : []),
  ];
                ...
                >
                  ...
                  {similar && similar.length > 0 ? (
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4 lg:gap-6">
                      {similar.slice(0, 12).map((show: any, index: number) => (
                        <motion.div
                          key={show.id}
                          initial={{ opacity: 0, y: 20 }}
                          animate={{ opacity: 1, y: 0 }}
                          transition={{ delay: index * 0.05, duration: 0.3 }}
                        >
                          <SimilarTVCard show={show} index={index} />
                        </motion.div>
                      ))}
                    </div>
                  ) : (
